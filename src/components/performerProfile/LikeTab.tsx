    ...
    const fetchPreferences = async () => {
      ...
      try {
        ...
        setSelectedFavorites({
          favoriteColor: fetchedProfile.favoriteColor ? (fetchedProfile.favoriteColor as string).split(FAVORITE_DELIMITER).map((s) => s.trim()).filter(Boolean) : [],
          favoriteCandies: fetchedProfile.favoriteCandies ? (fetchedProfile.favoriteCandies as string).split(FAVORITE_DELIMITER).map((s) => s.trim()).filter(Boolean) : [],
          favoriteBeverages: fetchedProfile.favoriteBeverages ? (fetchedProfile.favoriteBeverages as string).split(FAVORITE_DELIMITER).map((s) => s.trim()).filter(Boolean) : [],
          favoriteFood: fetchedProfile.favoriteFood ? (fetchedProfile.favoriteFood as string).split(FAVORITE_DELIMITER).map((s) => s.trim()).filter(Boolean) : [],
          favoriteMusic: fetchedProfile.favoriteMusic ? (fetchedProfile.favoriteMusic as string).split(FAVORITE_DELIMITER).map((s) => s.trim()).filter(Boolean) : [],
          languages: fetchedProfile.languages ? (fetchedProfile.languages as string).split(FAVORITE_DELIMITER).map((s) => s.trim()).filter(Boolean) : [],
        });
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedProfile: Partial<PerformerProfile> = {
        ...
        favoriteColor: (selectedFavorites.favoriteColor && selectedFavorites.favoriteColor.length > 0) ? selectedFavorites.favoriteColor.join(FAVORITE_DELIMITER) : null,
        favoriteCandies: (selectedFavorites.favoriteCandies && selectedFavorites.favoriteCandies.length > 0) ? selectedFavorites.favoriteCandies.join(FAVORITE_DELIMITER) : null,
        favoriteBeverages: (selectedFavorites.favoriteBeverages && selectedFavorites.favoriteBeverages.length > 0) ? selectedFavorites.favoriteBeverages.join(FAVORITE_DELIMITER) : null,
        favoriteFood: (selectedFavorites.favoriteFood && selectedFavorites.favoriteFood.length > 0) ? selectedFavorites.favoriteFood.join(FAVORITE_DELIMITER) : null,
        favoriteMusic: (selectedFavorites.favoriteMusic && selectedFavorites.favoriteMusic.length > 0) ? selectedFavorites.favoriteMusic.join(FAVORITE_DELIMITER) : null,
        languages: (selectedFavorites.languages && selectedFavorites.languages.length > 0) ? selectedFavorites.languages.join(FAVORITE_DELIMITER) : (profile?.languages ?? null),
    ...
    <div className="space-y-6">
      ...
      <div className="w-full">
        ...
        <textarea
          ...
          onChange={(e) => setProfileFields((prev) => ({ ...prev, showDescription: e.target.value.slice(0, 100) }))}
          rows={3}
          maxLength={100}
          ...
          {Object.entries(LikeService.getFavoriteOptions()).map(([field, options]) => (
            <div key={field}>
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from(new Set([...(options as string[]), ...((selectedFavorites as any)[field] || [])])).map((opt) => {
              ...
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  onChange={(e) => setNewFavoriteCustom((prev) => ({ ...prev, [field]: e.target.value }))}
      ...
      <div className="space-y-3">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">About me & preferences</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={field.key}>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setProfileFields((prev) => ({ ...prev, [field.key]: e.target.value.slice(0, 100) }))
                }
                rows={2}
                maxLength={100}
